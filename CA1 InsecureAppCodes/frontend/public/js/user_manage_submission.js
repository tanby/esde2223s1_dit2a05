        ...
        axios({
            ...
            .then(function(response) {
                ...
                for (let index = 0; index < records.length; index++) {
                    ...
                    let $card = $('<div></div>').addClass('card').attr('style', 'width:15rem;height:22rem');
                    $card.append($('<img></img>').addClass('card-img-top').addClass('app_thumbnail').attr('src', record.cloudinary_url));
                    let $cardBody = $('<div></div>').addClass('card-body');
                    let $editDesignButtonBlock = $('<div></div>').addClass('col-md-2 float-left');
                ...
                let totalPages = Math.ceil(totalNumOfRecords / 2);
